...
impl Hdma {
    ...
    /// Reads from the HDMA registers.
    pub fn read(&self, addr: u16) -> u8 {
        match addr {
            0xFF51 => self.read_source_high(),
            0xFF52 => self.read_source_low(),
            0xFF53 => self.read_dest_high(),
            0xFF54 => self.read_dest_low(),
            0xFF55 => self.read_mode_length(),
            _ => 0xFF,
        }
    }

    /// Writes to the HDMA registers.
    pub fn write(&mut self, addr: u16, value: u8) {
        match addr {
            0xFF51 => self.write_source_high(value),
            0xFF52 => self.write_source_low(value),
            0xFF53 => self.write_dest_high(value),
            0xFF54 => self.write_dest_low(value),
            0xFF55 => self.write_mode_length(value),
            _ => {}
        }
    }

    /// Returns whether an HDMA transfer is currently active.
    pub fn is_active(&self) -> bool {
        self.active
    }

    /// Writes to the source high byte register (HDMA1).
    pub fn write_source_high(&mut self, value: u8) {
        self.source_high = value;
    }

    /// Reads the source high byte register (HDMA1).
    pub fn read_source_high(&self) -> u8 {
        self.source_high
    }

    /// Writes to the source low byte register (HDMA2).
    pub fn write_source_low(&mut self, value: u8) {
        // Lower 4 bits are ignored (aligned to 16 bytes)
        self.source_low = value & 0xF0;
    }

    /// Reads the source low byte register (HDMA2).
    pub fn read_source_low(&self) -> u8 {
        self.source_low
    }

    /// Writes to the destination high byte register (HDMA3).
    pub fn write_dest_high(&mut self, value: u8) {
        // Only bits 0-4 are used (VRAM is 0x8000-0x9FFF)
        self.dest_high = value & 0x1F;
    }

    /// Reads the destination high byte register (HDMA3).
    pub fn read_dest_high(&self) -> u8 {
        self.dest_high
    }

    /// Writes to the destination low byte register (HDMA4).
    pub fn write_dest_low(&mut self, value: u8) {
        // Lower 4 bits are ignored (aligned to 16 bytes)
        self.dest_low = value & 0xF0;
    }

    /// Reads the destination low byte register (HDMA4).
    pub fn read_dest_low(&self) -> u8 {
        self.dest_low
    }

    /// Writes to the mode/length register (HDMA5).
    ...
    /// Reads the mode/length register (HDMA5).
    ...
    pub fn transfer_block(&mut self) -> Option<TransferRequest> {
